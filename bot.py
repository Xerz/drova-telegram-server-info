...
def send_sessions(update, context, edit_message=False):
    ...
    if response.status_code == 200:
        ...
        for i, session in enumerate(reversed(sessions), start=1):
            ...
            if finish_time:
                ...
                duration = str(datetime.timedelta(seconds=(session['finished_on'] - session['created_on']) / 1000))
        ...
        reply_markup=telegram.InlineKeyboardMarkup([[
                telegram.InlineKeyboardButton(text=update_text, callback_data='update_sessions')
            ]])
        if edit_message:
            # Modify the original message with the updated session list
            try:
                bot.edit_message_text(chat_id=chat_id, message_id=update.message.message_id, text=message, reply_markup=reply_markup, parse_mode=telegram.ParseMode.HTML)
            except telegram.TelegramError as e:
                pass

        else:
            # Send a new message with the updated session list
            bot.send_message(chat_id=chat_id, text=message, reply_markup=reply_markup, parse_mode=telegram.ParseMode.HTML)
...
def update_sessions_callback(update, context):
    ...
    if query.data == 'update_sessions':
        ...
        bot.send_message(chat_id=chat_id, text="Sorry, I don't understand that command.")
        query.answer() 
...
def handle_station(update, context):
    ...
    if len(context.args) > 0:
        ...
        user_id = requests.get('https://services.drova.io/accounting/myaccount', headers={'X-Auth-Token': auth_tokens.get(chat_id, '')}).json()["uuid"]
        # Retrieve a list of available server IDs from the API
        response = requests.get('https://services.drova.io/server-manager/servers', params={'user_id': user_id}, headers={'X-Auth-Token': auth_tokens.get(chat_id, '')})
        if response.status_code == 200:
            servers = response.json()
            # Create inline keyboard buttons for each available server ID
            keyboard = [[telegram.InlineKeyboardButton(text=s['name'], callback_data=f'set_server_id_{s["uuid"]}') for s in servers]]
            # Send a message to the user with the list of server IDs as inline keyboard buttons
            reply_markup = telegram.InlineKeyboardMarkup(keyboard)
            bot.send_message(chat_id=chat_id, text="Select a station:", reply_markup=reply_markup)
        else:
            bot.send_message(chat_id=chat_id, text=f"Error: {response.status_code}")
...
def products_data_update(update, context):
    ...
    if len(context.args) == 0: 
        ...
        response = requests.get('https://services.drova.io/product-manager/product/listfull2', params={}, headers={})
        if response.status_code == 200:
            games = response.json()
            ...
            bot.send_message(chat_id=chat_id, text=f"Game database has been updated from {products_data_len_old} games to {products_data_len_new}")
        else:
            bot.send_message(chat_id=chat_id, text=f"Error: {response.status_code}")
...
def set_server_id_callback(update, context):
    ...
    bot.answer_callback_query(callback_query_id=query.id, text=f"Station ID updated to {server_id}.")
...
def handle_command(update, context):
    ...
    if command == '/sessions':
        ...
        bot.send_message(chat_id=update.message.chat_id, text="Sorry, I don't understand that command.")

# Set up the message handler
def handle_message(update, context):
    bot.send_message(chat_id=update.message.chat_id, text="Sorry, I don't understand that message.")
...
def main():
    ...
    callback_query_handler = telegram.ext.CallbackQueryHandler(update_sessions_callback, pattern='^update_sessions$')
    dispatcher.add_handler(callback_query_handler)
    set_server_id_handler = telegram.ext.CallbackQueryHandler(set_server_id_callback, pattern='^set_server_id_')
